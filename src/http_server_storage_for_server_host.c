...
#endif
#ifndef HTTP_SERVER_HOST_RESPONSE_BUFFER_SIZE
#define HTTP_SERVER_HOST_RESPONSE_BUFFER_SIZE 8192
#endif
...
static uint8_t                       s_recv_buffers[HTTP_SERVER_HOST_CONNECTIONS]
                                                   [HTTP_SERVER_HOST_REQUEST_BUFFER_SIZE];
static uint8_t                       s_send_buffers[HTTP_SERVER_HOST_CONNECTIONS]
                                                   [HTTP_SERVER_HOST_RESPONSE_BUFFER_SIZE];
...
http_server_storage_t* http_server_storage_get_for_server_host(void)
{
    if (!s_initialized)
    {
        for (unsigned i = 0; i < HTTP_SERVER_HOST_CONNECTIONS; i++)
        {
            ...
            s_slots[i].recv_buffer_size = HTTP_SERVER_HOST_REQUEST_BUFFER_SIZE;
            s_slots[i].send_buffer_ptr  = s_send_buffers[i];
            s_slots[i].send_buffer_size = HTTP_SERVER_HOST_RESPONSE_BUFFER_SIZE;
        }
        s_initialized = 1;
    }
    return &s_storage;
