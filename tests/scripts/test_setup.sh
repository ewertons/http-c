...
sed -i "s@#define CLIENT_CERT_PATH \".*\"@#define CLIENT_CERT_PATH \"$(pwd)/certs/client.cert.pem\"@g" ../test_http_server.c
sed -i "s@#define CLIENT_PK_PATH \".*\"@#define CLIENT_PK_PATH \"$(pwd)/private/client.key.pem\"@g" ../test_http_server.c
sed -i "s@#define SERVER_CERT_PATH \".*\"@#define SERVER_CERT_PATH \"$(pwd)/certs/server.cert.pem\"@g" ../test_http_server.c
sed -i "s@#define SERVER_PK_PATH \".*\"@#define SERVER_PK_PATH \"$(pwd)/private/server.key.pem\"@g" ../test_http_server.c
sed -i "s@#define CA_CHAIN_PATH \".*\"@#define CA_CHAIN_PATH \"$(pwd)/certs/chain.ca.cert.pem\"@g" ../test_http_server.c

sed -i "s@#define SERVER_CERT_PATH \".*\"@#define SERVER_CERT_PATH \"$(pwd)/certs/server.cert.pem\"@g" ../../samples/main.c
sed -i "s@#define SERVER_PK_PATH \".*\"@#define SERVER_PK_PATH \"$(pwd)/private/server.key.pem\"@g" ../../samples/main.c
