...
/* Per-connection working memory. Both the slot and the buffer it points to
 * are owned by the caller (via #http_server_storage_t). The library never
 * allocates. */
typedef struct http_server_connection_slot
{
    ...
    /* Caller-owned per-connection buffer. */
    uint8_t*              buffer_ptr;
...
typedef struct http_server
{
    ...
    /* Single-threaded accept loop driven by an event_loop. The eventfd
     * `slot_freed_fd` lets a worker thread signal the accept loop when a
     * connection slot becomes available again, so the listener can be
     * re-armed. */
    event_loop_t           loop;
    int                    slot_freed_fd;
...
typedef struct http_server
{
    ...
    /* Single-threaded accept loop driven by an event_loop. The eventfd
     * `slot_freed_fd` lets a worker thread signal the accept loop when a
     * connection slot becomes available again. */
    event_loop_t           loop;
    int                    slot_freed_fd;
    bool                   listen_registered;
